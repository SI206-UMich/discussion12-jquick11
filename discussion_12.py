...
def create_employee_table(cur, conn):
    cur.execute('create table if not exist employees (employee_id INEGER PRIMARY KEY, first_name TEXT, job_id INTEGER, hire_date DATE, salary NUMBER ')
    conn.commit
...
def add_employee(filename, cur, conn):
    ...
    for item in data:
        ...
        job_id = item['job_id']
        salary = item['salary']
        cur.execute('insert or ignore into employees (employee_id, first_name, last_name, values, (?,?,?,?,?,?)')
    conn.commit
